 ...
 * @return array[]
 */
function theme_spacechild_user_preferences(): array {
    return [
        'drawer-open-block' => [
            'type' => PARAM_BOOL,
...
function theme_spacechild_get_extra_scss($theme) {
    ...
    if (theme_spacechild_get_setting('accessiblelinks') == 1) {
        $content .= '.underline--anim { @extend %underline-anim-accessible; }';
    }

    $forcefwvideo = theme_spacechild_get_setting('forcefwvideo');
    if ($forcefwvideo == 1) {
...
function theme_spacechild_get_main_scss_content($theme) {
    ...
    $customscss = $CFG->dirroot . '/theme/spacechild/scss/custum.scss';
    if (is_readable($customscss)) {
        $scss .= "\n" . file_get_contents($customscss);
    }

    $override = $CFG->dirroot . '/theme/spacechild/scss/frontpage-overrides.scss';
...
function theme_spacechild_get_pre_scss($theme) {
    ...
    $palette = $CFG->dirroot . '/theme/spacechild/scss/site-palette.scss';
    if (is_readable($palette)) {
        $scss .= "\n" . file_get_contents($palette);
    }
